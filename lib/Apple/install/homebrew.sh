...
package_bootstrap() {
  if [ ! -e "$homebrew_conf_install_homebrew_cmd" ]; then
    /bin/bash -c "$(curl $curl_flags -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)"
...
package_bootstrap_uninstall() {
  /bin/bash -c "$(curl $curl_flags -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/uninstall.sh)"
