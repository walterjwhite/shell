...
_buildfile_update_constants() {
  ...
  *)
  ...
  $GNU_SED -i 's|__APP_PLATFORM_PATH__|$APP_PLATFORM_PATH|g' $buildfile_output_package_file
  $GNU_SED -i 's|__APP_PATH__|$APP_PATH|g' $buildfile_output_package_file
  $GNU_SED -i 's|__APP_PLATFORM_PATH__|$APP_PLATFORM_PATH|g' $buildfile_output_package_file
...
_buildfile_replace_constants() {
  sed -e "s|__APP_PLATFORM_PATH__|$APP_PLATFORM_PATH|g" \
    -e "s|__APP_PATH__|$APP_PATH|g" \
    -e "s|__APP_PLATFORM_PATH__|$APP_PLATFORM_PATH|g" \
