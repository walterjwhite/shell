...
_git_app_dir_setup() {
  ...
  if [ -n "$is_git_repo" ]; then
    ...
    [ -z "$target_application_version" ] && {
      ...
      [ ! -e $APP_PLATFORM_PLATFORM/.app ] && exit_with_error "unable to determine application version for $target_application_name, .app not found"

      target_application_version=$(head -1 $APP_PLATFORM_PLATFORM/.app 2>/dev/null)
      target_application_build_date=$(head -2 $APP_PLATFORM_PLATFORM/.app 2>/dev/null | tail -1)
    }
  else
