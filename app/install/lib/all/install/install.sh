...
_install_prepare_target() {
  mkdir -p "$APP_PLATFORM_PATH/apps/$target_application_name"
}
...
_install_uninstall_filter_file_callback() {
  ...
  uninstall_packages=$($GNU_GREP -Pv '(^$|^#)' "$_SETUP_TYPE_FILE" | tr '\n' ' ')
...
_install_get_reinstalled_elements() {
  local setup_dir="$APP_INSTALL_PATH/setup/$target_platform"
...
_install_exclude_contents_from_file() {
  find "$registry_path/$target_application_name/$target_platform/setup" "$APP_PATH/*/type" -type f -name "*$setup_type_name*" \
...
_install_setup_type_append() {
  mkdir -p "$APP_PLATFORM_PATH/apps/$target_application_name/type"
  cat - >>"$APP_PLATFORM_PATH/apps/$target_application_name/type/${setup_type_name}"
}
