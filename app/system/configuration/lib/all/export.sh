...
_configuration_export() {
  ...
  [ ! -e $APP_DATA_PATH ] && exit_with_error "no configuration found at $APP_DATA_PATH"
...
_configuration_export_default() {
  ...
  tar -cp -C $APP_DATA_PATH/$provider_name . 2>/dev/null | tar -xp -C $configuration_tmpdir/$provider_name
}
