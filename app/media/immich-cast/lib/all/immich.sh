...
_query_images_for_year() {
  ...
  curl $conf_curl_flags -s -X POST \
    ...
    -d "{
      ...
      \"type\": \"IMAGE\"
    }" | jq -r '.assets.items[]? | select(.type == "IMAGE") | .id' | while read -r asset_id; do # NOTE: select() filter on JSON array — irreplaceable with coreutils; jq required
...
download_images() {
  ...
  while IFS= read -r url; do
    ...
    curl $conf_curl_flags -s -X GET "$url" \
      -H "x-api-key: ${conf_immich_memories_api_key}" \
