...
get_search_git_scope() {
  ...
  _path_in_path "$conf_git_project_base_path" && {
...
search_git_all() {
  if [ -z "$conf_git_project_base_path" ] && [ -z "$GIT_WORKSPACE_BASE_PATH" ] && [ -z "$DATA_PATH" ]; then
    search_path="."
  else
    search_path="$conf_git_project_base_path $GIT_WORKSPACE_BASE_PATH $DATA_PATH"
  fi
