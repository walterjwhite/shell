_lookup_package() {
  ...
  if [ -n "$package_category" ]; then
    lookup_search_result=$(curl $conf_curl_flags -s "https://packages.gentoo.org/packages/$package_category/$package_name" 2>/dev/null)
    printf '%s' "$lookup_search_result" | grep -q "kk-package-title"
    return
  fi

  lookup_search_result=$(curl $conf_curl_flags -s "https://packages.gentoo.org/packages/search?q=$package_name" 2>/dev/null)
