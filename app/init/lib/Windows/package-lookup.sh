_lookup_package() {
  ...
  lookup_search_result=$(curl $conf_curl_flags -s "$lookup_chocolatey_search_url" 2>/dev/null)
...
_lookup_locally() {
  command -v choco >/dev/null 2>&1 || {
    log_warn 'choco not available'
    return 1
  }

  choco search "^$package_name$" --local-only >/dev/null 2>&1 || {
