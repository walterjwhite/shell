_push_changes() {
  ...
  for app_platform in $(ls "$target_application_name"); do
    ...
    [ "$(git status --porcelain "$target_application_name/$app_platform" "$target_application_name/.app" | awk {'print$2'} | $GNU_GREP -Pv "$target_application_name/$app_platform/.app\$" | wc -l)" -eq 0 ] && {
    ...
    git add "$target_application_name/.app"
    git add "$target_application_name/$app_platform"
    git commit "$target_application_name/.app" "$target_application_name/$app_platform" -m "$app_version_tuple"
  done
...
_tag_published_build() {
  cd "$publish_wd" || exit_with_error "cannot return to publish working directory"
