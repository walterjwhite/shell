_buildfile_remove_unused_variables() {
  ...
  local errors
  if errors=$(sh -n "$buildfile_output_package_file" 2>&1); then
    :
  else
...
_buildfile_unused_variables() {
  ...
  if [ -z "$used_variables" ] || [ "$used_variables" = "()" ]; then
    candidates=$(_buildfile_list_variables)
  else
