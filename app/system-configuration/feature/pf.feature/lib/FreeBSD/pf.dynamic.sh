_pf_build_dynamic_schedules() {
	...
	local pf_dynamic_schedule_file=$(_mktemp)
...
_pf_clear_dynamic_schedules() {
	...
	$_CONF_GNU_SED -i '/pfctl -t/d' $existing_crontab
	$_CONF_GNU_SED -i '/firewall-update-/d' $existing_crontab
	$_CONF_GNU_SED -i '/# pf - anchor schedule/d' $existing_crontab
	$_CONF_GNU_SED -i '/# pf - dynamic schedule/d' $existing_crontab
	$_CONF_GNU_SED -i '/# pf - table schedule/d' $existing_crontab
