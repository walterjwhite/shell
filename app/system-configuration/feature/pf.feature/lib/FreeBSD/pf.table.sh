_pf_build_tables() {
	rm -rf $FIREWALL/group $FIREWALL/table/*.generated && mkdir -p $FIREWALL/group $FIREWALL/table
	_SYSTEM_CONFIGURATION_PATH_SED_SAFE=$(_sed_safe $_CONF_SYSTEM_CONFIGURATION_PATH/devices/)

	for table_file in $(find $_CONF_SYSTEM_CONFIGURATION_PATH -type f -name .tables); do
		_pf_build_table_for_group
	done
}

_pf_build_table_for_group() {
	group_path=$(dirname $table_file)
	group_name=$(printf '%s' "$table_file" | sed -e "s/$_SYSTEM_CONFIGURATION_PATH_SED_SAFE//" -e 's/\.tables//' -e 's/\//-/g' -e 's/-$//')

	pf_group_file=$FIREWALL/group/$group_name
	_pf_table_populate $group_path $pf_group_file

	local table_line table_name pf_table_file table_start table_end index
	local pf_table_crontab_file=$(_mktemp)

	local pf_wrote_table_schedule_header

	_DETAIL "building table schedule - $group_name"

	$_CONF_GNU_GREP -Pvh '^($|#)' $table_file | while read table_line; do
		...
		_DETAIL "building table schedule - $group_name - $table_name - $table_line"
...
_pf_table_populate() {
	...
	find $1 -type f ! -name 'tables' ! -name '*device' \
		-exec $_CONF_GNU_GREP -Poh '^IP=.*' {} + |
		sed -e 's/^IP=//' | sort -u >>$2
}

_pf_table_static() {
	cat $pf_group_file >>$FIREWALL/table/$1.generated
}
